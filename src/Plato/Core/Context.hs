...
getTypeFromContext :: MonadThrow m => Span -> Context -> Int -> m Ty
...
getVarIndex :: MonadThrow m => Span -> Context -> Name -> m Int
getVarIndex sp ctx x = case elemIndex x (vmap fst ctx) of
        Just i -> return i
