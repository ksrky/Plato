...
data Bind
        = ValueBind Ident LExpr
        | TypeBind Ident Kind LType
        | ModBind Ident Mod
        deriving (Eq, Show)
...
instance Pretty Bind where
        pretty (ValueBind id exp) = hsep [pretty id, colon, equals, pretty exp]
        pretty (TypeBind id kn ty) = hsep [pretty id, colon, pretty kn, equals, pretty ty]
        pretty (ModBind id mod) = hsep [pretty id, colon, equals, pretty mod]