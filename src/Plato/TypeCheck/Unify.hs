...
unify :: (MonadReader ctx m, HasUnique ctx, MonadIO m, MonadThrow m) => Span -> Tau -> Tau -> m ()
unify sp = unify'
    where
        unify' :: (MonadReader ctx m, HasUnique ctx, MonadIO m, MonadThrow m) => Tau -> Tau -> m ()
        ...
        unify' ty1 ty2 = throwLocErr sp $ vsep ["Couldn't match type.", "Expected type:" <+> pretty ty2, indent 2 ("Actual type:" <+> pretty ty1)]

        unifyVar :: (MonadReader ctx m, HasUnique ctx, MonadIO m, MonadThrow m) => MetaTv -> Tau -> m ()
        ...
        occursCheck tv1 ty2 = do
                tvs2 <- getMetaTvs ty2
                when (tv1 `S.member` tvs2) $ throwLocErr sp $ hsep ["Infinite type:", squotes $ pretty ty2]

        badType :: Tau -> Bool
        badType (VarT (BoundTv _)) = True
        badType _ = False

unifyFun :: (MonadReader ctx m, HasUnique ctx, MonadIO m, MonadThrow m) => Span -> Rho -> m (Sigma, Rho)
