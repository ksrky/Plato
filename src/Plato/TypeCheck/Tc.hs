...
checkType ::
        (MonadReader ctx m, HasEnv ctx, HasUnique ctx, MonadIO m, MonadThrow m) =>
        LExpr ->
        Type ->
        m LExpr
...
instPatSigma ::
        (MonadReader ctx m, HasUnique ctx, MonadIO m, MonadThrow m) =>
        Sigma ->
        Expected Sigma ->
        m Coercion
...
subsCheck sigma1 sigma2 = do
        ...
        unless (null bad_tvs) $ throwUnexpErr $ hsep ["Subsumption check failed: ", pretty sigma1 <> comma, pretty sigma2]
        return $ deepskolTrans skol_tvs coercion1 coercion2

subsCheckRho :: (MonadReader ctx m, HasUnique ctx, MonadIO m, MonadThrow m) => Sigma -> Rho -> m Coercion
...
subsCheckFun :: (MonadReader ctx m, HasUnique ctx, MonadIO m, MonadThrow m) => Sigma -> Rho -> Sigma -> Rho -> m Coercion
...
instSigma :: (MonadReader ctx m, HasUnique ctx, MonadIO m, MonadThrow m) => Sigma -> Expected Rho -> m Coercion
