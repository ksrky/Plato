...
-- | Declarations
topdecls    :: { [ LTopDecl Name ] }
            : topdecls ';' topdecl                  { $3 : $1 }
            | topdecl                               { [$1] }

topdecl     :: { LTopDecl Name }
...
constrs     :: { [(LName, [LType Name])] }
            : constr '|' constrs                    { $1 : $3 }
            | constr                                { [$1] }

constr      :: { (LName, [LType Name]) }
            : lcon types                            { ($1, $2) }
            | '(' lconop ')' types                  { ($2, $4) }
...
atype       :: { LType Name }
            ...
            | conid                                 { cL $1 (ConT (mkLName tyconName $1)) }
            | qconid                                { cL $1 (ConT (mkLName tyconName $1)) }
            | tyvar                                 { cL $1 (VarT $1) }
...
-- | Alternatives
alts        :: { [(LPat Name, LExpr Name)] }
            : alt ';' alts                          { $1 : $3 }
            | alt                                   { [$1] }
            | {- empty -}                           { [] }

alt         :: { (LPat Name, LExpr Name) }
            : pat '->' expr                         { ($1, $3) }
...
setFixities :: MonadThrow m => [LName] -> Located Int -> FixDir -> ParserT m (Decl Name)
