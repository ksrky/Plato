...
data Tok = TExp LExpr | TOp Path Fixity deriving (Eq, Show)

resolveFixity :: MonadThrow m => FixityEnv -> LExpr -> m LExpr
resolveFixity fixenv = resolve

linear :: (MonadReader env m, HasFixityEnv env, MonadThrow m) => LExpr -> m [Tok]
linear (L _ (OpE e1 p e2)) = do
        e1' <- linear e1
        e2' <- linear e2
        ...
        fix <-
                asks getFixityEnv
                        >>= ( \case
                                ...
                                . M.lookup id
        return $ e1' ++ [TOp p fix] ++ e2'
linear e = do
        e' <- resolve e
        return [TExp e']

resolve :: (MonadReader env m, HasFixityEnv env, MonadThrow m) => LExpr -> m LExpr
resolve (L sp exp) =
        L sp <$> case exp of
                VarE{} -> return exp
                AppE e1 e2 -> do
                ...
                e@OpE{} -> do
                        ...
                        case res of
                                ...
                                _ -> throwPsErr sp "Error at parsing infix expression"
                    where
                        ...
                        parse op1@(Fixity prec1 fix1) e1 ((TOp name op2@(Fixity prec2 fix2)) : rest)
                                | prec1 == prec2 && (fix1 /= fix2 || fix1 == Nonfix) = throwPsErr sp "Error at parsing infix expression"
