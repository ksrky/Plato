...
elabTypDefn :: forall e m. (MonadReader e m, HasUniq e, MonadIO m) => T.TypDefn -> m [C.Entry]
...
elabDefns ::
    (MonadReader e m, HasCoreEnv e, HasUniq e, MonadIO m, MonadThrow m) =>
    [T.Defn 'T.Typed] -> m [C.Entry]
elabDefns decs = do
...
typToCore = catchErrors . applyContext . elabDefns
