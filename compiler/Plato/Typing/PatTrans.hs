...
constructors :: forall e m. (MonadReader e m, HasTypEnv e, MonadThrow m, MonadIO m) => Type -> m [(Path, [Type])]
constructors ty = getCon [] =<< zonk ty
    where
        getCon :: [Type] -> Type -> m [(Path, [Type])]
        getCon acc (AppT fun arg) = getCon (unLoc arg : acc) (unLoc fun)
        getCon acc (ConT tc) = do
                (params, constrs) <- find tc =<< asks getTypEnv
                mapM (\(con, tys) -> (liftScope tc con,) <$> mapM (substTvs params (reverse acc)) tys) constrs
...
isVarorSameCon :: Path -> Clause 'Typed -> Bool
...
match ::
        ...
        Clauses 'Typed ->
        m (Expr 'Typed)
match [(var, ty)] [] = return (CaseE (VarE $ PIdent var) ty [])
...
matchVar (var, _) rest clauses = do
        let clauses' = (`map` clauses) $ \case
                ...
                (L _ (VarP vp) : pats, exp) -> (pats, substExpr vp (VarE $ PIdent var) exp)
...
matchCon (var, ty) rest clauses = do
        ...
        return $ CaseE (VarE $ PIdent var) ty alts

choose :: Path -> Clauses 'Typed -> Clauses 'Typed
...
matchClause (con, arg_tys) vts clauses = do
        ...
        clauses' <- forM clauses $ \case
                ...
                (L _ (VarP v) : ps', e) -> do
                        let con_exp = foldl AppE (VarE con) (map (VarE . PIdent) args)
