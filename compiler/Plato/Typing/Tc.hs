...
checkPat (L sp pat) exp_ty = tcPat pat exp_ty
  where
    ...
    instPatSigma_ sigma exp_ty = catches (instPatSigma sigma exp_ty) (tcErrorHandler sp sigma exp_ty)
...
checkRho ::
    (MonadReader e m, HasTypEnv e, HasUniq e, MonadIO m, MonadCatch m) =>
    LExpr 'Untyped -> Rho -> m (Expr 'Typed)
checkRho exp ty = tcRho exp (Check ty)

inferRho ::
    (MonadReader e m, HasTypEnv e, HasUniq e, MonadIO m, MonadCatch m) =>
    LExpr 'Untyped -> m (Expr 'Typed, Rho)
inferRho exp = do
...
tcRho ::
    ...
    LExpr 'Untyped -> Expected Rho -> m (Expr 'Typed)
tcRho (L sp exp) exp_ty = tcRho' exp exp_ty
  where
    ...
    tcRho' (AbsE var (Just (L _ var_ty)) body) (Check exp_ty) = do
        ...
        return $ AbsE var var_ty (substExpr var (unCoer coer $ VarE var) body')
...
inferSigma ::
    (MonadReader e m, HasTypEnv e, HasUniq e, MonadIO m, MonadCatch m) =>
    LExpr 'Untyped -> m (Expr 'Typed, Sigma)
inferSigma exp = do
...
checkSigma ::
    (MonadReader e m, HasTypEnv e, HasUniq e, MonadIO m, MonadCatch m) =>
    LExpr 'Untyped -> Sigma -> m (Expr 'Typed)
checkSigma exp sigma = do
    ...
    unless (null bad_tvs) $ throwLocErr (getLoc exp) $
        vsep
...
tcBinds (Nonrec (L _ (Bind (id, Nothing) exp))) = do
    ...
    return $ Nonrec $ Bind (id, sigma) exp'
