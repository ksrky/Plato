...
module Plato.Typing.Kc (checkKind, checkKindStar, inferKind, kcTypDefns) where
...
kcTypDefn ::
    (MonadReader e m, HasTypEnv e, HasUniq e, MonadCatch m, MonadIO m) =>
    XTypDefn 'Untyped -> m (XTypDefn 'Typed)
...
kcTypDefns ::
    (MonadReader e m, HasTypEnv e, HasUniq e, MonadCatch m, MonadIO m) =>
    XTypDefns 'Untyped -> m (XTypDefns 'Typed)
...
kcTypDefns (Mutrec tdefs) = do
    ...
    local (modifyTypEnv $ extendList envbinds) $ Mutrec <$> mapM kcTypDefn tdefs
