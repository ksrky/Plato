...
decNamesUnique = loop
  where
    ...
    loop [] = return ()
    loop (id1 : ids) =
        case Data.List.find (id1 ==) ids of
            Just id2 ->
                throwLocErr (getLoc id2) $ hsep ["Multiple declarations for", squotes $ pretty id2]
            Nothing -> loop ids
...
argNamesUnique ids = do
    ...
    case dup of
        [] -> return ()
        (id1, id2) : _ -> throwLocErr (getLoc id1 <> getLoc id2)
                            $ hsep ["Conflicting definitions for", squotes $ pretty id2]

argPatsUnique :: (MonadThrow m) => [LPat] -> m ()
argPatsUnique pats = let ids = getDomain pats in argNamesUnique ids
...
dataConType id (con, ty) = loop1 ty
  where
    ...
    loop1 (L _ (BinT _ op _)) | nameIdent id == nameIdent op = return ()
    loop1 ty = loop2 ty
    loop2 :: (MonadThrow m) => LType -> m ()
    loop2 (L _ (AppT ty1 _)) = loop2 ty1
    loop2 (L _ (ConT id2)) | nameIdent id == nameIdent id2 = return ()
    loop2 (L sp ty) = throwLocErr sp
                        $ hsep ["Data constructor", squotes $ pretty con
                               , "returns", pretty ty, "instead of", pretty id]
...
checkNumArgs (FunBindD id clauses@((pats1, _) : _))
    ...
    | otherwise = throwLocErr (getLoc clauses)
                    $ hsep [ "Each clause of the definition of"
                           , pretty id, "has different number of arguments."]
